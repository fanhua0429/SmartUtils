...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(SGPageTitleViewConfigure *)configure {
    if (self = [super initWithFrame:frame]) {
        ...
        if (configure == nil) {
            configure = [SGPageTitleViewConfigure pageTitleViewConfigure];
...
- (void)setupTitleButtons {
    // 计算所有按钮的文字宽度
    CGFloat paddingX = 15;
    self.allBtnTextWidth = paddingX*2;
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { // SGPageTitleView 静止样式
        ...
        for (NSInteger index = 0; index < titleCount; index++) {
            ...
            CGFloat btnX = btnW * index + paddingX;
...
//        NSLog(@"%@>>>%@ ,progress - %@ ,SG_x - %@",@(originalBtn.tag),@(targetBtn.tag),@(progress),@(self.indicatorView.SG_x));
    } else {
        /// 1、计算 indicator 偏移量
        // targetBtn 文字宽度
